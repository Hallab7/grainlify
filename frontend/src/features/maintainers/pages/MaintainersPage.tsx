...
export function MaintainersPage({ onNavigate }: MaintainersPageProps) {
  ...
  const loadProjects = async () => {
    ...
    try {
      ...
      // Clear projects on error so we show error message instead of empty state
      setProjects([]);
    } finally {
      setIsLoading(false);
    }
      ...
      <div className={`backdrop-blur-[40px] rounded-[20px] border p-2 relative z-10 transition-colors ${
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="relative z-50">
            <button
              className={`flex items-center gap-3 px-5 py-3 rounded-[14px] backdrop-blur-[30px] border transition-all group ${
                theme === 'dark'
            ...
            >
              ...
              <ChevronDown className={`w-4 h-4 transition-all ${isRepoDropdownOpen ? 'rotate-180' : ''} ${
            ...
            {isRepoDropdownOpen && (
              <div className={`absolute top-full left-0 mt-2 w-[380px] rounded-[20px] border-2 z-50 overflow-hidden transition-colors ${
                ...
                {/* Header */}
                <div className={`px-6 py-5 border-b-2 bg-gradient-to-b from-white/10 to-transparent transition-colors ${
                    ...
                    groupedRepositories.map((group) => {
                      ...
                      return (
                        <div key={group.org}>
                          ...
                          <button 
                            className={`w-full px-6 py-3.5 flex items-center justify-between transition-all group/repo ${
                              theme === 'dark' 
                                ? 'hover:bg-[#4a3e30]' 
                                : 'hover:bg-[#c9b8a0]'
                            }`}
                          ...
                          >
                            <div className="flex items-center gap-3 flex-1">
                              <span className={`text-[15px] font-bold group-hover/repo:text-[#c9983a] transition-colors ${
                                theme === 'dark' ? 'text-[#e8dfd0]' : 'text-[#2d2820]'
                              }`}>
                                {group.org}
                              </span>
                              {group.repos.length === 0 && (
                                <span className={`text-[12px] italic font-medium transition-colors ${
                                  theme === 'dark' ? 'text-[#b8a898]' : 'text-[#8a7b6a]'
                                }`}>
                                  No synced repos
                                </span>
                              )}
                          ...
                          {group.repos.length > 0 && isExpanded && (
                            <div className={`py-2 space-y-1 transition-colors ${
                              theme === 'dark' ? 'bg-[#2d2820]/30' : 'bg-[#c9b8a0]/30'
                            }`}>
                              {group.repos.map((repo) => (
                                <label
                                  key={repo.id}
                                  className={`flex items-center gap-3 px-6 py-2.5 rounded-[10px] mx-4 cursor-pointer group/subrepo transition-all ${
                                    theme === 'dark' 
                                      ? 'hover:bg-[#4a3e30]' 
                                      : 'hover:bg-[#c9b8a0]'
                                  }`}
                ...
                <div className={`px-6 py-5 border-t-2 bg-gradient-to-t from-white/10 to-transparent transition-colors ${
                  theme === 'dark' ? 'border-white/20' : 'border-white/30'
                }`}>
                  <button 
                    onClick={() => setIsAddModalOpen(true)}
                    className={`w-full flex items-center justify-between px-5 py-3.5 rounded-[14px] border-2 transition-all group/add ${
                      theme === 'dark'
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <span className={`text-[14px] font-bold group-hover/add:text-[#c9983a] transition-colors ${
                        theme === 'dark' ? 'text-[#e8dfd0]' : 'text-[#2d2820]'
                      }`}>
                        Add a repository
                      </span>
                    </div>
                    <SettingsIcon className={`w-4 h-4 group-hover/add:text-[#c9983a] transition-colors ${
                      theme === 'dark' ? 'text-[#b8a898]' : 'text-[#7a6b5a]'
                    }`} />
                  </button>
                </div>
